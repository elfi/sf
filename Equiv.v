...
Proof.
    ...
    aexp_cases (induction a) Case; simpl;
        ...
        (* APlus, AMinus, AMult *)
        try ((* two rewrites *)
             rewrite -> IHa1; try rewrite -> IHa2;
             (* finishing off the main subgoal *)
             try reflexivity;
             (* solving two subgoals generated by the rewrites *)
             inversion H; assumption).
    Case "AId". 
        inversion H. apply update_neq. assumption.
Qed.
...
| E_WhileLoop: forall (b1 : bexp) (st st' st'' : state) (c1 : com),
    beval st b1 = true ->
    c1 / st || st' ->
    (WHILE b1 DO c1 END) / st' || st'' ->
    (WHILE b1 DO c1 END) / st || st''
...
Example havoc_example1:
    (HAVOC X) / empty_state || update empty_state X 0.
